...
; Realiza la busqueda de un simbolo dentro de una lista de simbolos de un ambiente.
(define rib-find-position 
  (lambda (sym los)
    (list-find-position sym los)))

; Función aux que encuentra la pos de un simb.
(define list-find-position
  (lambda (sym los)
    (list-index (lambda (sym1) (eqv? sym1 sym)) los)))

; Función aux recursiva que retorna el valor del index.
(define list-index
  (lambda (pred ls)
    (cond
      ((null? ls) #f)
